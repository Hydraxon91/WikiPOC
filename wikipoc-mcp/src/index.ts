...
async function tryRelogin(headers: Record<string, string>): Promise<boolean> {
...
async function authFetch(path: string, init: RequestInit & { token?: string }, retry = true): Promise<any> {
  const headers: Record<string, string> = { ...(init.headers as Record<string, string> || {}) };
  if (init.token) headers["Authorization"] = "Bearer " + init.token;
  if (!headers["Content-Type"] && init.method !== "DELETE") headers["Content-Type"] = "application/json";
...
async function postJson(path: string, body: any, token?: string) {
  return authFetch(path, { method: "POST", token, body: JSON.stringify(body) } as any);
}

async function patchJson(path: string, body: any, token?: string) {
  const init: any = { method: "PATCH", token };
  if (body !== undefined) init.body = JSON.stringify(body);
  return authFetch(path, init);
}

async function putJson(path: string, body: any, token?: string) {
  return authFetch(path, { method: "PUT", token, body: JSON.stringify(body) } as any);
}

async function del(path: string, token?: string) {
  return authFetch(path, { method: "DELETE", token } as any);
}
...
server.tool(
  ...
  { postTitle: z.string(), content: z.string(), forumTopicId: z.string(), userProfileId: z.string(), userName: z.string() },
  wrap(async (args: any) => {
